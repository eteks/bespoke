...
class Catalog extends CI_Model {
	...
	{	
		...
		$this->db->insert('shopping_category', $data);
	...
	{	
		...
		$this->db->update('shopping_category', $data);
	...
	{	
		// Query to insert data in database
		$this->db->insert('shopping_recipient', $data);
		...
		foreach($category_data as $key => $value) {
			...
			$this->db->insert('shopping_recipient_category', $recipient_category_map);
		}
		if ($this->db->affected_rows() > 0) {
			return true;
		}
	...
	{	
		$query['recipient_data'] = $this->db->get_where('shopping_recipient', array('recipient_id' => $id))->row_array();
		// $query['subcategory_category'] = $this->db->get_where('shopping_subcategory_category', array('subcategory_mapping_id' => $id))->result_array();
		$this->db->select('category_mapping_id');
		$this->db->from('shopping_recipient_category');
	...
	{	
		...
		foreach($category['category_data'] as $value) {
			...
			$query = $this->db->get();
			if($query->num_rows() == 0)
				$this->db->insert('shopping_recipient_category', $recipient_category_map_data);
		}
		$this->db->where('recipient_id', $recipient_data['recipient_id']);
		$this->db->update('shopping_recipient', $recipient_data);
		// trans_complete() function is used to check whether updated query successfully run or not
		if ($this->db->trans_complete() == false) {
			return false;
		}
	...
	{	
		//get list of categories from database using mysql query 
		$query = $this->db->query("SELECT * FROM shopping_product_attribute order 
	...
	{	
		...
		$this->db->select('*');
		$this->db->from('shopping_product_attribute');
		$this->db->where($condition);
		// $this->db->limit(1);
		$query = $this->db->get();
		if ($query->num_rows() == 0) {
			// Query to insert data in database
			$this->db->insert('shopping_product_attribute', $data);
			if ($this->db->affected_rows() > 0) {
				return true;
			}
		} else {
	...
	{	
		...
		if ($query->num_rows() > 0) {
			...
			$this->db->update('shopping_product_attribute', $data);
			return true;
		}	
	}
	public function get_product_attribute_data($id)
	{	
		$query = $this->db->get_where('shopping_product_attribute', array('product_attribute_id' => $id));
		return $query->row_array();
	}
	public function get_products()
	{	
		//get list of products from database using mysql query 
		// $query = $this->db->query("SELECT * FROM shopping_product order 
		// 	by product_createddate desc");		
		// echo "<pre>";
		// print_r($query->result());
		// echo "</pre>";
		$this->db->select('pro.*,cat.category_name,subcat.subcategory_name,rec.recipient_type');
		$this->db->from('shopping_product AS pro');
		$this->db->join('shopping_category AS cat', 'cat.category_id = pro.product_category_id', 'Left');
		$this->db->join('shopping_subcategory AS subcat', 'subcat.subcategory_id = pro.product_subcategory_id', 'Left');
		$this->db->join('shopping_recipient AS rec', 'rec.recipient_id = pro.product_recipient_id', 'Left');
		$this->db->order_by('product_createddate','desc');
		$query['product_result'] = $this->db->get()->result_array();

		$this->db->select('*');
		$this->db->from('shopping_product AS pro');
		$this->db->join('shopping_product_upload_image AS img', 'img.product_mapping_id = pro.product_id', 'inner');
		$this->db->order_by('product_createddate','desc');
	...
	{	
		...
		$this->db->select('*');
		$this->db->from('shopping_product');
		$this->db->where($condition);
		// $this->db->limit(1);
		$query = $this->db->get();
		if ($query->num_rows() == 0) {
			// Query to insert data in database
			$this->db->insert('shopping_product', $data_product_basic);
			//get inserted subcategory id to map in subcategory category relationship table
			...
			if(!empty($data_product_attributes)){
				foreach($data_product_attributes as $key=>$value){
					...
					foreach ($attributes as $key => $value) {
						...
						$this->db->insert('shopping_product_attribute_value', $product_attributes_data);
					...
					$this->db->insert('shopping_product_attribute_group', $product_attributes_group);
				}
			}		
			if ($this->db->affected_rows() > 0) {
				return true;
	...
	public function get_product_attribute_sets(){
		$query = $this->db->query("SELECT *
				FROM `shopping_product_attribute_value` AS c
				INNER JOIN 
				(
					...
					FROM shopping_product_attribute_group t
				...
				(
					...
					FROM shopping_product_attribute
				)AS pa ON pa.product_attribute_id = c.product_attribute_id
				INNER JOIN
				(
					SELECT product_id,product_title from shopping_product
	...
	{	
		...
		$this->db->select('*');
		$this->db->from('shopping_product AS pro');
		$this->db->join('shopping_category AS cat', 'cat.category_id = pro.product_category_id', 'inner');
		$this->db->join('shopping_subcategory AS subcat', 'subcat.subcategory_id = pro.product_subcategory_id', 'inner');
		...
		// $this->db->select('*');
		// $this->db->from('shopping_product_attribute_group AS progroup');
		// $this->db->where('product_mapping_id', $id);
		// $query['product_attribute_list'] = $this->db->get()->result_array();

		$query['product_attribute_list'] = $this->db->query("SELECT * FROM shopping_product_attribute_value AS c INNER JOIN ( SELECT product_attribute_group_id,product_attribute_group_price,	product_mapping_id,product_attribute_group_totalitems, SUBSTRING_INDEX( SUBSTRING_INDEX( t.product_attribute_value_combination_id, ',', n.n ) , ',', -1 ) value FROM shopping_product_attribute_group t CROSS JOIN numbers n WHERE n.n <=1 + ( LENGTH( t.product_attribute_value_combination_id ) - LENGTH( REPLACE( t.product_attribute_value_combination_id, ',', ''))) AND t.product_mapping_id=$id) AS a ON a.value = c.product_attribute_value_id INNER JOIN shopping_product_attribute AS pa ON c.product_attribute_id=pa.product_attribute_id")->result_array();
		return $query;
	}
		...
		if(!empty($data_product_attributes_exists)){
			foreach($data_product_attributes_exists as $key=>$value){
				...
				foreach ($attributes as $key => $value1) {
					...
					// echo "</pre>";
					$this->db->select('*');
					$this->db->from('shopping_product_attribute_value');
				...
				$this->db->select('*');
				$this->db->from('shopping_product_attribute_group');
				$this->db->where('product_attribute_group_id', $product_attribute_group_id);
				$query1 = $this->db->get();
				if($query1->num_rows() > 0){
					$this->db->where('product_attribute_group_id', $product_attribute_group_id);
					$this->db->update('shopping_product_attribute_group', $product_attributes_group);
				}
				else{
					$this->db->insert('shopping_product_attribute_group', $product_attributes_group);
				}
